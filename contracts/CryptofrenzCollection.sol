...
// Cryptofrenz implementation of ERC721A standard with some minor optimization of unnecessary logic by @h_cryptogonist

pragma solidity 0.8.0;

/**
 * @dev Custom Cryptofrenz implementation of https://eips.ethereum.org/EIPS/eip-721[ERC721] Non-Fungible Token Standard, including
 * the Metadata extension (excl. Enumerable). Built to optimize for lower gas during batch mints.
 *
...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
    ...
    uint256 private mintingIndex = 0;
    ...
    // An empty address value does not necessarily mean the token is unowned. See ownershipOf implementation for details.
    ...
    */
    function ownerOf(uint256 tokenId) public view override returns (address) {
        ...
        for (uint256 i = tokenId; ; i--) {
            address tokenOwnership = _owners[i];
    ...
    */
    function _safeMint(
        ...
        uint256 mintingTokenId = mintingIndex;
        require(to != address(0), "ERC721A: mint to the zero address");

        _beforeTokenTransfers(address(0), to, mintingTokenId, quantity);

        _balances[msg.sender] += quantity;
        _owners[mintingTokenId] = to;

        for (uint256 i = 0; i < quantity; i++) {
            emit Transfer(address(0), to, mintingTokenId + i);
        }

        mintingIndex += quantity;
        _afterTokenTransfers(address(0), to, mintingTokenId, quantity);
    }
    ...
    */
    function _transfer(
        ...
        // Update _balances and _owners records
        _balances[from] -= 1;
        _balances[to] += 1;
        _owners[tokenId] = to;
 ...
 * @dev Extends ERC721A Cryptofrenz implementation by @h_cryptogonist
 */
...
contract CryptoFrenzCollection is ERC721A, Ownable {
    ...
    constructor(string memory name_, string memory symbol_, string memory baseURI) ERC721A(name_, symbol_) {
        setBaseURI(baseURI);
    }

   function recoverSigner(bytes memory signature) private view returns (address) {
    ...
    */
    ...
    function setBaseURI(string memory baseURI) public onlyOwner {
        require(!_metadataIsFreezed, "You cannot switch freezed metadata URI");
    ...
    */
    function freezeMetadata() external onlyOwner returns (bool) {
        require(_metadataIsFreezed == false, "You cannot unfreeze metadata");
    ...
    */
    function freezeSaleState() external onlyOwner returns (bool) {
        require(_saleStateIsFreezed == false, "You cannot unfreeze sale state");
    ...
    */
    function mintFromWhitelist(uint256 numberOfTokens, bytes memory signature) external payable {
        require(!_saleStateIsFreezed, "Sales are freezed for this contract");
        require(saleIsActive, "Sale must be active to mint cards");
        require(numberOfTokens <= whitelistCanMint,
        "Purchase would exceed max tokens available to mint from whitelist"
        );
        require(recoverSigner(signature) == whitelistKey,
        "Address is not whitelisted or used wrong signature"
        );
        require(!signatureUsed[signature], "Whitelist signature cannot be used twice");
        require(totalSupply() + numberOfTokens <= 1420 && reservedTokenCount + numberOfTokens <= 100,
        "Purchase would exceed max supply of Cards"
        );
        require(whitelistMintPrice.mul(numberOfTokens) <= msg.value,
    ...
    */
    function mintCard(uint256 numberOfTokens) external payable {
        require(!_saleStateIsFreezed, "Sales are freezed for this contract");
        require(saleIsActive, "Sale must be active to mint cards");
        require(numberOfTokens <= opensaleCanMint, "Purchase would exceed max tokens available to mint on opensale");
        require(totalSupply() + numberOfTokens <= 1420 && openSaleTokenCount + numberOfTokens <= 1320,
        "Purchase would exceed max supply of Cards"
        );
        require(mintPrice.mul(numberOfTokens) <= msg.value,
        "Ether value sent is not correct"
        );
    ...
    */
    function mintReservedCardTo(address to, uint256 numberOfTokens) external onlyOwner {
        require(!_saleStateIsFreezed, "Sales are freezed for this contract");
        if (to == address(0)) {
            to = msg.sender;
        }
